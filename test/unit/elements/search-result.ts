...
import {createSearchResultInstance} from '../../../src/app/elements/search/search-result';

export default function ({test}: Assert) {
    test('corner search result', t => {

        const createCorner = (lng = 1234, lat = 4321, street1 = 'st1', street2 = 'st2', municipality = 'playa'): CornerSearchResult => ({
            type: 'corner',
            geometry: {
                type: 'Point',
                coordinates: [lng, lat]
        ...
        t.test(`toString() should return the detialed corner`, t => {
            ...
            t.eq(createSearchResultInstance(corner).toString(),
                `esquina st1 y st2, Marianao`);
        });
    ...
    test('street search result', t => {
        ...
        const createStreet = (name: string, municipality = 'Playa'): StreetSearchResult => ({
            type: 'street',
            name,
            geometry: {
                type: 'LineString',
    ...
    test('street_block search result', t => {
        const createStreetBlock = (): BlockSearchResult => ({
            'type': 'street_block',
            'name': 'Calle 10',
            'municipality': 'Playa',
    ...
    test('point_of_interest search result', t => {
        const createPointOfInterest = (): PointOfInterestSearchResult => ({
    ...
    test('lng_last search result', t => {
        const createGeoCoordSearchResult = (): GeoCoordSearchResult => createGeoCoord(-82.420269, 23.125608);
