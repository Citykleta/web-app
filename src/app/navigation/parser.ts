...
export const fromUrlToState = (url: URL): ApplicationState => {
    try {
        ...
        if (location) {
            ...
            state.map.zoom = Number(zoom.slice(-1));
        }
...
// rough serialization could be optimized as we shall know the data structure (to be check too we backward compatibility issues in case of structure changes)
const serializeData = (state: ApplicationState): string => btoa(JSON.stringify(project(state)));
const deserializeData = (encoded: string) => atob(encoded);
